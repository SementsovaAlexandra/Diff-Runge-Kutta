    ...
    internal static class Program
    {
        ...
        static void ModeFile(string fileName)
        {
            ...
            try
            {
                ...
                var result = solver.Solve(problem, (prog, current) =>
                {
                    Console.WriteLine($"Iteration {prog}%: t={current.T} of ({problem.T0},{problem.T1})");
                });
                ...
                {
                    Transport<SolutionRequest>.SendStream(fileStream, result, false);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            var problem = Problem.Random(size);
